...
public:
	...
	bool AStarSearchverticies(Node* startNode, Node* endNode, std::list<Node*>& path, float Heuristic)
	{
		//Let openList be a List of Nodes
		//	Let closedList be a List of Nodes
		//	Set startNode.parent to null
		...
		while (!openList.empty())
		{
			...
			for (Edge* edge : currentNode->Connections)
			{
				...
				if (!(std::find(closedList.begin(), closedList.end(), edge->m_target) != closedList.end()))
				{
					...
					//what I am trying to do is find the vert closest to the target and set that as the 
					float shortest =9999999;
					Vector2 verty;
					for (auto ver : edge->m_target->vertices)
					{
						if(edge->m_target->Pos.distance(ver) < shortest)
						{
							shortest = edge->m_target->Pos.distance(ver);
							verty = ver;
						}
					}
					auto hScore = edge->m_target->Pos.distance(verty);
					auto fScore = gScore + hScore;
