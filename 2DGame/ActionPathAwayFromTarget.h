...
inline eBehaviourResult ActionPathAwayFromTarget::execute(GameObject* gameObject, float deltaTime)
{
	if (gameObject->target != nullptr)
	{
		if (gameObject->target->isfish())
		{
			...
			{
				if (gameObject->smoothPath.front().distance(gameObject->hunter->position) <= gameObject
				                                                                             ->position.distance(
					                                                                             gameObject
					                                                                             ->hunter->position))
				{
