...
inline eBehaviourResult CheckInRangeOfFood::execute(GameObject* gameObject, float deltaTime)
{
	foodinrange.clear();

	for (auto object : *gameObject->ObjectPool)
	{
		if (object->isfood() && object->team == gameObject->team && object->position.distance(gameObject->position) <
			range)
		{
